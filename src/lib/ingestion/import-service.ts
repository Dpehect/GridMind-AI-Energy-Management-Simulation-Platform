...
export async function importReadings(rows: ReadingImportRow[]) {
  ...
  const meterMap = new Map(meters.map((meter: { id: string; serialNumber: string }) => [meter.serialNumber, meter.id]));
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.activityLog.create({ data: { workspaceId: (await tx.workspace.findFirstOrThrow()).id, action: "READINGS_IMPORTED", entityType: "Reading", entityId: `batch-${Date.now()}`, metadata: { inserted, skipped, source: "csv" } } });
  });
